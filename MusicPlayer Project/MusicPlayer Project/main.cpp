...
int main(){
    ...
    bool switchCase;
    std::cout << "Enter Yes to start the program\n"; 
    std::cin >> switchCase;
    //Make this not case senstiive
    while(switchCase == true){
        int checl = -1;
        std::cin >> checl;
        switch(checl){
            case 0:
            {
            soundObject v1{0};
            std::cout << "Enter a new case number 1 to change the song if you want" << std::endl;
            std::cin >> checl;
            }
            case 1:
            {soundObject v2{3};
            std::string newcheck;
            std::cout << "Do you want to end the program Enter no\n";
            std::cin >> newcheck;
            if(newcheck == "no"){
                switchCase = false;
            }
    ...
    std::cout << "Ending the Music Player Visit soon\n";
    return 0;
}
